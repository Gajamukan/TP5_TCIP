...
void ScanPec12 (bool ValA, bool ValB, bool ValPB)
{
   ...
   if((DebounceIsReleased(&DescrA) == 1)&& (DebounceGetInput(&DescrB) == 0))
    {
      Pec12.Inc = 1;
      Pec12.Dec = 0;
   ...
   else if((DebounceIsReleased(&DescrB) == 1)&& (DebounceGetInput(&DescrA) == 0))
   {
      Pec12.Inc = 0;
      Pec12.Dec = 1;
   ...
   {
       Pec12ClearInactivity();
       if(DebounceIsReleased(&DescrPB) == 1)
       {
           Pec12.OK = 1;              // événement action OK
           DebounceClearPressed(&DescrPB);
       }
       else if(Pec12.PressDuration >= 500)
       {
           DebounceClearPressed(&DescrPB);
           Pec12.ESC = 1;             // événement action ESC
       }
       else
       {
           Pec12.PressDuration++;
